...
import CarrosselChevette from "@/components/CarrosselChevette";
...
export default function ChevettePage() {
  ...
  useEffect(() => {
    const fetchRifaChevette = async () => {
      try {
        // Buscar especificamente a rifa do Chevette
        const response = await fetch("/api/rifas");
        if (!response.ok) {
          throw new Error("Erro ao carregar rifas.");
        }
        const rifas: RifaComTickets[] = await response.json();
        const rifaChevette = rifas.find(r => r.titulo.includes("Chevette"));

        if (!rifaChevette) {
          throw new Error("Rifa do Chevette não encontrada.");
        }

        setRifa(rifaChevette);
      } catch (err) {
